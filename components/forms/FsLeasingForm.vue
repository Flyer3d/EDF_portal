<template>
    <v-container grid-list-xl flat class="fs-leasing-form">
        <v-layout align-center column v-if="loaded">
            <v-flex>
                <v-card style="width: 100%">
                    <v-form ref="form" v-model="valid" lazy-validation>
                        <v-card-text class="fs-leasing-form__body">
                            <v-layout row wrap>
                                <v-flex xs12>
                                    <h1 class="my-5 text-xs-center">Leasing</h1>
                                </v-flex>
                                ...
                                <v-flex xs12>
                                    <component
                                            :is="fields['Предмет лизинга'].fieldType"
                                            label="Leased Item"
                                            ref="Предмет лизинга"
                                            :field="fields['Предмет лизинга']"
                                            :inputData.sync="formData['Предмет лизинга']"
                                    ></component>
                                </v-flex>
                                <!-- ********************* -->
                                <v-flex lg6 xs12>
                                    <component
                                            :is="fields['Первоначальный взнос (USD)'].fieldType"
                                            label="Initial Fee"
                                            suffix="$"
                                            ref="Первоначальный взнос (USD)"
                                            :field="fields['Первоначальный взнос (USD)']"
                                            :inputData.sync="formData['Первоначальный взнос (USD)']"
                                    ></component>
                                </v-flex>
                                <v-flex lg6 xs12></v-flex>
                                <!-- ********************* -->
                                ...
                                <div class="fs-leasing-form__divider mt-2"></div>
                                <!-- ********************* -->
                                <v-flex lg6 xs12>
                                    <component
...
<script>
    ...
    const modelName = 'Заявка на лизинг';
    const processName = 'Подать заявку на лизинг';

    const defaultFormData = {
            'Выручка в год (USD)': '',
            'Предмет лизинга': '',
            'Стоимость предмета (USD)': '',
            'Первоначальный взнос (USD)': '',
