<template>
    <v-container grid-list-xl flat class="fs-investment-form">
        <v-layout align-center column v-if="loaded">
            <v-flex>
                <v-card style="width: 100%">
                    <v-form ref="form" v-model="valid" lazy-validation>
                        <v-card-text class="fs-investment-form__body">
                            <v-layout row wrap>
                                <v-flex xs12>
                                    <h1 class="my-5 text-xs-center">Investment and Project<br>
                                        Financing</h1>
                                </v-flex>
                                ...
                                <v-flex lg6 xs12>
                                    <component
                                            :is="fields['Сумма контракта (USD)'].fieldType"
                                            label="Contract Amount (USD)"
                                            suffix="$"
                                            ref="Сумма контракта (USD)"
                                            :field="fields['Сумма контракта (USD)']"
                                            :inputData.sync="formData['Сумма контракта (USD)']"
                                    ></component>
                                </v-flex>
                                <v-flex lg6 xs12>
                                    <component
                                            :is="fields['Размер собственных средств (USD)'].fieldType"
                                            label="Amount of Own Funds (USD)"
                                            suffix="$"
                                            ref="Размер собственных средств (USD)"
                                            :field="fields['Размер собственных средств (USD)']"
                                            :inputData.sync="formData['Размер собственных средств (USD)']"
                                    ></component>
                                </v-flex>
                                <!-- ********************* -->
                                <v-flex lg6 xs12>
                                    <component
                                            :is="fields['Документация по проекту'].fieldType"
                                            label="Project Documentation"
                                            ref="Документация по проекту"
                                            :field="fields['Документация по проекту']"
                                            :inputData.sync="formData['Документация по проекту']"
                                    ></component>
                                </v-flex>
                                <v-flex lg6 xs12>
                                </v-flex>
                                ...
                                <div class="fs-investment-form__divider mt-2"></div>
                                <!-- ********************* -->
                                <v-flex lg6 xs12>
                                    <component
...
<script>
    ...
    const modelName = 'Заявка на инвестиционное и проектное финансирование';
    const processName = 'Подать заявку на инвестиционное и проектное финансирование';

    const defaultFormData = {
            'Выручка в год (USD)': '',
            'Сумма контракта (USD)': '',
            'Размер собственных средств (USD)': '',
